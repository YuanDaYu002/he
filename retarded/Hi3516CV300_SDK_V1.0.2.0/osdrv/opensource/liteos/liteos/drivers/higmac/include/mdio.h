...
#define higmac_mdio_readl(ld, ofs)    \
        ({ unsigned long reg = readl((ld)->gmac_iobase + (ofs)); \
        higmac_trace(2, "readl(0x%04X) = 0x%08lX", (ofs), reg); reg; })

#define higmac_mdio_writel(ld, v, ofs)    \
    do { writel(v, (ld)->gmac_iobase + (ofs));\
        higmac_trace(2, "writel(0x%04X) = 0x%08lX", (ofs),\
                (unsigned long)(v)); \
    } while (0)
...
#define higmac_mdio_writel_bits(ld, v, ofs, bits_desc) \
    do { \
        ...
        unsigned long _reg  = higmac_mdio_readl(ld, ofs); \
