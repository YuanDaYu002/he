...
#define mmc_ms_to_ticks(ms)      LOS_MS2Tick(ms)
...
static inline uint32_t  mmc_swab32(uint32_t x)
{
    uint32_t __x = (x);
    return (uint32_t)(
...
#ifndef le32_to_cpu
#define le32_to_cpu(x)  (mmc_swab32(x))
#endif
