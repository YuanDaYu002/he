...
#define _THREAD_PRIVATE_MUTEX_LOCK(name)  \
	(void)pthread_mutex_lock( &__THREAD_NAME(name)._private_lock )
#define _THREAD_PRIVATE_MUTEX_UNLOCK(name) \
	(void)pthread_mutex_unlock( &__THREAD_NAME(name)._private_lock )

/* Note that these aren't compatible with the usual OpenBSD ones which lazy-initialize! */
#define _MUTEX_LOCK(l) (void)pthread_mutex_lock((pthread_mutex_t*) l)
#define _MUTEX_UNLOCK(l) (void)pthread_mutex_unlock((pthread_mutex_t*) l)
