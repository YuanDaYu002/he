...
#define STRNCPY_SM(dest, destMax, src, count) \
    (\
    (NULL != (void*)dest && NULL != \
    (void*)src && (size_t)destMax >0 && (((UINT64T)(destMax) & (UINT64T)(-2)) < SECUREC_STRING_MAX_LEN) && (TWO_MIN(count , strlen(src)) + 1) <= (size_t)destMax ) ?  \
    ( (count < strlen(src))? (memcpy(dest, src, count), *((char*)dest + count) = '\0', EOK) :( memcpy(dest, src, strlen(src) + 1), EOK ) ) :(strncpy_error(dest, destMax, src, count))  \
    )

#define STRCPY_SM(dest, destMax, src) \
    (\
    ( NULL != (void*)dest && NULL != \
    (void*)src  && (size_t)destMax >0 && (((UINT64T)(destMax) & (UINT64T)(-2)) < SECUREC_STRING_MAX_LEN) && ( strlen(src) + 1) <= (size_t)destMax )? \
    (memcpy(dest, src, strlen(src) + 1), EOK) :( strcpy_error(dest, destMax, src))\
    )
