...
static struct ZK_VECTOR zks_vector[] = {
    {
        {0}, 0, NULL,
        "/jffs0/ASCII_vector_w1920"
    },
    {
        {0}, 0, NULL,
        "/jffs0/ASCII_vector_w960"
    },
    {
        {0}, 0, NULL,
        "/jffs0/ASCII_vector_w480"
    }
};
...
返回：
    成功：0
    失败：-1
注意：使用结束后需要释放: free(ret_info->matrix_data)
*/
int str2matrix_vector(HLE_U8 *str, OSD_matrix_info_t* ret_info,int enc_w ,int enc_h )
{
    ...
    int matrix_width,matrix_height;//字符串OSD点阵数据（宽度+高度）像素点个数（bit位数）
