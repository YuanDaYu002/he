...
#include "PPCS_Type.h"
#include "../../../../../include/media_server_signal_def.h"
#include "../../../../../include/typeport.h"
	...
	{	
		...
		else //PPCS_Connect连接成功
		{
			...
			if (0 > g_Session)
			{
				...
				/****发送命令获取实时流********************************************/
				if(1 == index )//0:Lan 1:是P2P连接     2:RLY
				{
					// 5. do job ...
					// 填充信令数据结构
					printf("into Real-time stream transmission!\n");
					char * buf = (char*)malloc(sizeof(STRUCT_OPEN_LIVING_REQUEST));
					memset(buf,0,sizeof(STRUCT_OPEN_LIVING_ECHO));
					...
					*/
					ret = PPCS_Write(g_Session, 0, (char*)mycmd, sizeof(STRUCT_OPEN_LIVING_REQUEST));
					if (ret < 0)
					{
					...
					{	
						printf("PPCS_Write ok ! sizeof(STRUCT_OPEN_LIVING_REQUEST)(%ld)bytes  write(%d)bytes\n",sizeof(STRUCT_OPEN_LIVING_REQUEST),ret);

					}

					//发送成功，等待接收码流数据
					int ReadSize = 1024*1024*1;
					...
					while(1)
					{
						...
						ret = PPCS_Read(g_Session, 0, (CHAR*)&video_buffer, &ReadSize, 200);
						if(ret < 0)
						{
						...
						{
							printf("PPCS_Read %d bytes!   ",ret);
						}
						

						ret = fwrite(video_buffer,ret,1,video_file);
						if(ret < 0)
						{
							printf("fwrite error !\n");
							return NULL;
	...
	unsigned char NumberOfServer = 1;
		...
		else
		{
			...
			printf("into here 001\n");
				...
				char *packageVersion = (char*)"0.0.0.1";
				//URL[256];
				//char *url = "http://soft1.xzstatic.com/KMSpico_Install_DownZa.Cn.rar";
				char *url = (char*)"http://speedtest.tokyo.linode.com/100MB-tokyo.bin";
				...
				注意：  	channel 是CH_CMD
				*/
				ret = PPCS_Write(g_Session, CH_CMD, (char*)mycmd, sizeof(STRUCT_SET_UPDATE_REQUEST));
				if (ret < 0)
				{
					st_info("PPCS_Write: gSessionID=%d, CH=%d, ret=%d [%s]\n", g_Session, 0, ret, getP2PErrorCodeInfo(ret));
					return -1;
				...
				{	
					printf("PPCS_Write ok ! sizeof(STRUCT_SET_UPDATE_REQUEST) = (%ld) bytes , write (%d) bytes\n",sizeof(STRUCT_SET_UPDATE_REQUEST),ret);
